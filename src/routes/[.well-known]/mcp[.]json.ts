import { createFileRoute } from "@tanstack/react-router";

/**
 * Endpoint discovery for MCP clients and crawlers that probe well-known paths
 * before attempting a connection.
 */
export const Route = createFileRoute("/.well-known/mcp.json")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        const origin = new URL(request.url).origin;
        return new Response(
          JSON.stringify(
            {
              name: "agent-nexus",
              title: "Agent Nexus",
              description:
                "Continuously verified registry of the APIs, MCP servers and CLIs that AI agents call.",
              version: "0.3.0",
              transport: "streamable-http",
              endpoint: `${origin}/mcp`,
              authorization: {
                type: "oauth2.1",
                required_for: ["submit_entry", "vote_entry", "report_invocation", "list_my_submissions"],
                anonymous_read: true,
              },
              servers: [
                {
                  name: "agent-nexus-read",
                  title: "Agent Nexus (read-only, no auth)",
                  transport: "streamable-http",
                  endpoint: `${origin}/api/public/mcp`,
                  authorization: { type: "none" },
                  tools: [
                    "discover_capabilities",
                    "search_registry",
                    "get_entry",
                    "list_categories",
                  ],
                  intended_for: "fully autonomous agents with no human to approve a consent screen",
                },
                {
                  name: "agent-nexus",
                  title: "Agent Nexus (full)",
                  transport: "streamable-http",
                  endpoint: `${origin}/mcp`,
                  authorization: { type: "oauth2.1" },
                },
              ],
              self_service_key: {
                endpoint: `${origin}/api/public/keys`,
                method: "POST",
                body: { agent: "your-agent-name" },
                header: "x-api-key",
                purpose: "raise the daily call quota without any human step",
              },
              anonymous_surfaces: {
                mcp_read_only: `${origin}/api/public/mcp`,
                self_service_key: `${origin}/api/public/keys`,
                catalog_text: `${origin}/llms.txt`,
                directives: `${origin}/agents.txt`,
                search: `${origin}/api/public/registry`,
                discover: `${origin}/api/public/discover?need=`,
                capabilities: `${origin}/api/public/capabilities`,
                bulk_ndjson: `${origin}/api/public/entries.ndjson`,
                report: `${origin}/api/public/report`,
                openapi: `${origin}/openapi.json`,
                plugin_manifest: `${origin}/.well-known/ai-plugin.json`,
                agent_card: `${origin}/.well-known/agent.json`,
                feed: `${origin}/feed.xml`,
              },
              documentation: `${origin}/connect`,
            },
            null,
            2,
          ),
          {
            headers: {
              "Content-Type": "application/json",
              "Access-Control-Allow-Origin": "*",
              "Cache-Control": "public, max-age=300",
            },
          },
        );
      },
    },
  },
});
